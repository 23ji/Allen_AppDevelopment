...
final class ViewController: UIViewController {
    ...
    private lazy var emailTextFieldView: UIView = {
        ...
        // 🍑 회색 뷰 위에 서브뷰를 올려줘야함! 가장 마지막에 써진 코드가 제일 위에 올라감
    ...
    private lazy var stackView: UIStackView = { //🍑스택뷰 만들기
        let stview = UIStackView(arrangedSubviews: [emailTextFieldView, passwordTextFieldView, loginButton])
        stview.spacing = 18
        stview.axis = .vertical //🍑 축을 수직(세로로) 설정
        stview.distribution = .fillEqually //🍑 간격 동일하게
        stview.alignment = .fill //🍑 풀로 정렬
        return stview
    }()
    ...
    private func setupAutoLayout() {
        ...
        // 🍑 위의 방법대로 한번에 true 처리해도 되고 아래처럼 하나씩 설정해도 됨(처음에는 하나씩 설정해보고 익숙해지면 한번에 처리하기)
//        emailTextField.translatesAutoresizingMaskIntoConstraints = false
